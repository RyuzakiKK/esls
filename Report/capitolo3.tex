...
\begin{itemize}
 ...
 \item impedire l'autenticazione dei client tramite password, ma consentirla solo tramite chiavi asimmetriche (\textit{PasswordAuthentication no; ChallengeResponseAuthentication no; PubkeyAuthentication yes});
...
\subsection{Automatizzazione e deployment}
...
\\Queste caratteristiche sono di grande importanza in un sistema di questo tipo, in cui i Raspberry sono dislocati sul territorio e collegati in rete.
...
Questo garantisce una facile lettura e la modifica anche manuale dei file. I file così creati hanno estensione \textit{cfg} e contengono quattro sezioni principali con diversi campi ciascuna.
...
Ci si può accorgere facilmente di un guasto di questo tipo perché nei log sul server centrale si noterebbe la mancanza degli eventi di accensione e rilevamento auto da parte del sistema embedded guasto.
...
Il sito Web è stato realizzato seguendo uno stile minimale, moderno e intuitivo per l'utente, con controlli sull'input che facilitano l'individuazione di errori (figura \ref{WD}).
L'interfaccia è ottimizzata sia per un uso desktop che mobile.
\begin{figure}[tbp]
	\centering
	\includegraphics[scale=.52]{figure/web_desktop.png}
...
il sito è composto da un'unica pagina Web, di conseguenza uno strumento come SASS non sarebbe stato di particolare aiuto, e avrebbe inutilmente aggiunto complessità.
