...
I dettagli implementativi dell'interfaccia web per controllare i Raspberry sono indicati in seguito nella sezione \ref{serv_rest_in}.
...
\section{Implementazione della servlet \label{serv_impl}}
...
\subsection{Interfaccia REST in ingresso \label{serv_rest_in}}
...
Il contenuto dei messaggi è descritto nella parte di dettaglio del Time Series Database (sezione \ref{serv_rest_in}).
...
Un database relazionale potenzialmente potrebbe offri prestazioni maggiori rispetto ad un file di testo, ma tale vantaggio sarebbe evidente solo nel caso in cui i dati da salvare fossero in numero considerevole, e non è questo il caso;
...
Quando i dispositivi embedded eseguono azioni rilevanti, inviano ad un'istanza della servlet (sezione \ref{serv_impl}) un messaggio \textit{JSON} per consentire agli addetti ai lavori di avere una panoramica generale sull'andamento dell'intero sistema.
I possibili campi del messaggio sono stati descritti nella sezione \ref{serv_rest_in} relativa all'implementazione della servlet.
...
\begin{figure}[tbp]
	...
	\caption{Interfaccia web Desktop \label{FSM POLICY}} %TODO questa label è sbagliata.
