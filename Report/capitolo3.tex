...
\paragraph{SSH}
Il servizio SSH (Secure Shell) è utilizzato da tutti i tecnici che intendono essettuare modifiche più avanzate allo scheduling dei lampioni rispetto a quelle che si potrebbero fare attraverso l'interfaccia Web (come per esempio modificare lo scheduling di diversi lampioni tutti in una volta).
Come per la configurazione di SSH lato server spiegata nel paragrafo \ref{ssh-server}, anche qui sono state aggiunte le stesse impostazioni di hardening rispetto alla configurazione di default.

\subsection{Linguaggio di programmazione, paradigma ad attori e organizzazione a task}
Per il programma in esecuzione sui Raspberry Pi si è scelto di utilizzare Python 3, che garantisce un elevato supporto per i sensori e attuatori attualmente disponibili, essendo esso il linguaggio di riferimento per la programmazione su Raspberry.
Altri linguaggi come Java o C potrebbero attualmente avere un supporto più limitato su diversi sensori e attuatori.
...
Essendo questo un progetto che prevede l'utilizzo a regime di centinaia o migliaia di dispositivi embedded dislocati sul territorio, il guasto Hardware deve essere considerato la norma e non l'eccezione.
Per questo motivo sono stati presi in considerazione i vari tipi di guasti che potrebbero verificarsi che vanno dal guasto dell'intera board embedded, guasto di rete fino al guasto di un singolo lampione.
Nel caso un dispositivo embedded smettesse di funzionare solo i lampioni da esso gestiti non si accenderebberto più.
Questo perché il sistema nel complesso è in grado di funzionare indipendentemente dai singoli dispositivi che lo compongono.
Ci si può accorgere facilmente di un gasto di questo tipo perché nei log sul server centrale si noterebbe subito la mancanza degli eventi di accensione e rilevamento auto da parte del sistema embedded guasto.
...
Se invece si fulminasse un singolo LED e il dispositivo embedded fosse in grado di accorgersene potrebbe avvertire il server centrale inviando un determinato tipo di messaggio.
